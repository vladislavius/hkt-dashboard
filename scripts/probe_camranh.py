"""Разведка camranh.aero — перехватывает все XHR и печатает endpoint'ы.

Запуск:  python3 scripts/probe_camranh.py

Цель: найти URL и формат ответа API, отдающего таблицу рейсов CXR.
"""
from playwright.sync_api import sync_playwright
import json

URLS = [
    "https://camranh.aero/ru/flights/search-flight/departure-flight",
    "https://camranh.aero/ru/flights/search-flight/arrival-flight",
]
...
def probe():
    with sync_playwright() as p:
        ...
        captured = []  # (url, method, status, ctype, body_preview)
        ...
        for url in URLS:
            ...
            try:
                ...
                page.wait_for_timeout(3000)  # подождать доп XHR
            except Exception as e:
                print(f"⚠️ goto error: {e}")

        # Выбираем наиболее интересные (исключим страничные HTML-ответы)
        api_like = [
            c for c in captured
        ...
        # Также покажем HTML страниц — вдруг там hidden JSON/data-attr
