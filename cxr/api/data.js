...
// CXR API endpoint — отдаёт cxr/data/ (аналог /api/data.js для HKT, но свои пути).
module.exports = (req, res) => {
  ...
  let filePath;
  const baseDir = path.join(process.cwd(), 'cxr', 'data');
  ...
  if (!fs.existsSync(filePath)) {
    res.status(404).json({ error: 'CXR data not found for this date.' });
    return;
  }
