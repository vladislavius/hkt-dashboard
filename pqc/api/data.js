...
// PQC API endpoint — отдаёт pqc/data/ (аналог /api/data.js для HKT, но свои пути).
module.exports = (req, res) => {
  ...
  let filePath;
  const baseDir = path.join(process.cwd(), 'pqc', 'data');
  ...
  if (!fs.existsSync(filePath)) {
    res.status(404).json({ error: 'PQC data not found for this date.' });
    return;
  }
