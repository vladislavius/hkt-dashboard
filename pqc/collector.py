"""Phu Quoc (PQC, Vietnam) flight data collector — isolated from HKT/CXR/DAD.

Pipeline (2 sources, первая успешная побеждает):
1. phuquocairport.com HTML таблицы — основной. Прямой HTTP без CF/auth:
   GET https://www.phuquocairport.com/flight-status-arrivals-departures/
   GET https://www.phuquocairport.com/flight-status-departures/
2. Aviationstack API с IATA=PQC — fallback (последний резерв, качество плохое).

Storage:
- pqc/data/accumulated_YYYY-MM-DD.json (merged flight lists)
- pqc/data/dashboard.json (сводка для фронта)
- Supabase pqc_flight_daily (см. scripts/migrate_supabase_pqc.sql)

Telegram: префикс [PQC] в сообщении.
"""
import os
import json
import re
...
PQC_DATA_DIR = 'pqc/data'
SUPABASE_TABLE = 'pqc_flight_daily'
TG_PREFIX = '[PQC]'

PHUQUOC_BASE = 'https://www.phuquocairport.com'
PHUQUOC_ARR_URL = f'{PHUQUOC_BASE}/flight-status-arrivals-departures/'
PHUQUOC_DEP_URL = f'{PHUQUOC_BASE}/flight-status-departures/'
...
# ── phuquocairport.com HTML source ───────────────────────────────────────
# Row regex — non-greedy match per <tr>…</tr>
_ROW_RE = re.compile(
    r'<tr[^>]*>\s*'
    r'<td[^>]*data-label="Scheduled"[^>]*>([^<]*)</td>\s*'
    r'<td[^>]*data-label="Revised"[^>]*>([^<]*)</td>\s*'
    r'<td[^>]*data-label="(?:From|To)"[^>]*>([^<]*)\s*\(([A-Z]{3})\)\s*</td>\s*'
    r'<td[^>]*data-label="Flight"[^>]*>(.*?)</td>\s*'
    r'<td[^>]*data-label="Status"[^>]*>([^<]*)</td>',
    re.I | re.S,
)
# Flight: "Vietnam Airlines VN 1835" or "Jeju Air 7C* 2315" or with <br>Codeshare: ...
...
def fetch_flights_phuquoc(date_str):
    """Тянет arrivals + departures HTML с phuquocairport.com.
    Возвращает (arrivals_list, departures_list) списки словарей или (None, None).
    Примечание: сайт показывает только today (±незначительный rollover).
    """
    arr_html = _fetch_html(PHUQUOC_ARR_URL)
    dep_html = _fetch_html(PHUQUOC_DEP_URL)
    if arr_html is None and dep_html is None:
        return None, None
    return (_parse_rows(arr_html or "", "arrival"),
            _parse_rows(dep_html or "", "departure"))


def _parse_rows(html, direction):
    rows = []
    for m in _ROW_RE.finditer(html):
        scheduled, revised, city_name, iata, flight_raw, status = m.groups()
        scheduled = (scheduled or "").strip()
        revised = (revised or "").strip()
        city_name = (city_name or "").strip()
        iata = (iata or "").strip().upper()
        flight_raw = re.sub(r'<[^>]+>', ' ', flight_raw or "").strip()
        # "Codeshare:" suffix — отрежем
        flight_main = flight_raw.split("Codeshare")[0].strip()
        fm = _FL_RE.match(flight_main)
        if fm:
            airline = fm.group(1).strip()
            al_iata = fm.group(2).replace("*", "")
            fnum = fm.group(3)
            fn = f"{al_iata} {fnum}"
        else:
            airline = flight_main
...
# phuquocairport.com status → категория
_PQC_COMPLETED = ('arrived', 'landed', 'departed', 'diverted')
_PQC_CANCELLED = ('cancel',)  # "Cancelled", "Canceled"


def analyze_phuquoc(flights, direction, date_str):
    """Нормализует разобранные row → {date_str: {count, pax, countries, stats, flight_list}}."""
    cnt, pax_total = 0, 0
    ...
    for f in flights or []:
        ...
        status_lc = (f.get("status") or "").lower()
        if any(k in status_lc for k in _PQC_CANCELLED):
            st["cancelled"] += 1
        elif any(k in status_lc for k in _PQC_COMPLETED):
            st["completed"] += 1
        else:
            st["upcoming"] += 1

        # ISO-like time "2026-04-20T20:00" from "20:00"
        sched = f.get("scheduled") or ""
        est = f.get("revised") or ""
        sched_iso = f"{date_str}T{sched}" if re.fullmatch(r"\d{2}:\d{2}", sched) else ""
        est_iso = f"{date_str}T{est}" if re.fullmatch(r"\d{2}:\d{2}", est) else ""
        ...
        rec = {
            ...
            "status": status_lc,
            "pax": flight_pax,
            "dep_time": sched_iso if direction == "departure" else (est_iso or ""),
            "arr_time": sched_iso if direction == "arrival"   else (est_iso or ""),
        }
    ...
    return {date_str: {"count": cnt, "pax": pax_total, "countries": dict(ctry),
                       "stats": st, "flight_list": flight_list, "source": "phuquocairport"}}


# ── Aviationstack source (fallback) ──────────────────────────────────────
def get_api_key():
...
# ── Period aggregation (identical logic to CXR, paths adjusted) ──────────
def load_period_stats(today_str, days):
    ...
    for i in range(days):
        ...
        dt_str = dt.isoformat()
        fp = Path(PQC_DATA_DIR) / f"accumulated_{dt_str}.json"
        if not fp.exists():
            continue
        try:
...
def run():
    ...
    print("📡 Пробуем phuquocairport.com HTML...")
    a_raw, d_raw = fetch_flights_phuquoc(today)
    if a_raw is not None and d_raw is not None and (len(a_raw) > 0 or len(d_raw) > 0):
        a_res = analyze_phuquoc(a_raw, "arrival",   today)
        d_res = analyze_phuquoc(d_raw, "departure", today)
        source = "phuquocairport"
        print(f"✅ phuquocairport: {len(a_raw)} arrivals raw, {len(d_raw)} departures raw")
    ...
    # Accumulate merge
    Path(PQC_DATA_DIR).mkdir(parents=True, exist_ok=True)
    acc_file = Path(PQC_DATA_DIR) / f"accumulated_{today}.json"
    acc = {"date": today,
    ...
    yest_file = Path(PQC_DATA_DIR) / f"accumulated_{yesterday}.json"
    if yest_file.exists():
    ...
    available_dates = sorted([p.stem.replace("accumulated_", "")
                              for p in Path(PQC_DATA_DIR).glob("accumulated_*.json")])
    ...
    Path(PQC_DATA_DIR, "dashboard.json").write_text(json.dumps(dashboard_data, ensure_ascii=False, indent=2))
    print(f"✅ {PQC_DATA_DIR}/dashboard.json обновлён.")
